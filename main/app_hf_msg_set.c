...
HF_CMD_HANDLER(conn)
{
    ...
    connection_retry_count = 0;
    printf("Connection mode: Waiting for headset to connect (AG mode)...\n");
    printf("Make sure your headset is trying to connect.\n");
    printf("If headset was connecting, it should now establish SLC.\n");
    connection_in_progress = true;
    
    // In AG mode, we mainly wait for incoming connections
    // If needed, we can also try outbound, but this often causes collision
    // For now, just keep AG discoverable and let headset connect
    esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_GENERAL_DISCOVERABLE);
    return 0;
}

// Retry connection after collision
HF_CMD_HANDLER(con_retry)
{
    if (!connection_in_progress) {
        printf("No connection in progress. Use 'con' to start connection.\n");
        return 0;
    }
    
    if (connection_retry_count >= MAX_RETRY_COUNT) {
        printf("Max retries (%d) exceeded. Please try 'con' again to restart.\n", MAX_RETRY_COUNT);
    ...
    printf("Retrying connection (attempt %d/%d)...\n", connection_retry_count + 1, MAX_RETRY_COUNT);
    printf("Waiting 3 seconds before retry...\n");
...
HF_CMD_HANDLER(reset)
{
    printf("Resetting connection state...\n");
    is_connected = false;
    connection_in_progress = false;
    connection_retry_count = 0;
    esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_GENERAL_DISCOVERABLE);
    printf("Connection state reset. Ready for new connection.\n");
    return 0;
}
...
HF_CMD_HANDLER(disc)
{
    if (!is_connected && !connection_in_progress) {
        printf("Not connected or waiting.\n");
        return 0;
    }
    ...
    // Stay discoverable and connectable for future connections
    esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_GENERAL_DISCOVERABLE);
...
static hf_msg_hdl_t hf_cmd_tbl[] = {
    {"con", hf_conn_handler},
    {"dis", hf_disc_handler},
    {"con_retry", hf_con_retry_handler},
    {"reset", hf_reset_handler},
    {"scan", hf_scan_handler},
...
static char *hf_cmd_explain[] = {
    ...
    "scan for Bluetooth devices (10 seconds)",
    "reset connection state (use if connection fails)",
...
void register_hfp_ag(void)
{
    ...
    const esp_console_cmd_t HF_ORDER(scan) = {
        .command = "scan",
        .help = hf_cmd_explain[HF_CMD_IDX_SCAN],
        .hint = NULL,
        .func = hf_cmd_tbl[HF_CMD_IDX_SCAN].handler,
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&HF_ORDER(scan)));

    const esp_console_cmd_t HF_ORDER(reset) = {
        .command = "reset",
        .help = hf_cmd_explain[HF_CMD_IDX_RESET],
        .hint = NULL,
        .func = hf_cmd_tbl[HF_CMD_IDX_RESET].handler,
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&HF_ORDER(reset)));
